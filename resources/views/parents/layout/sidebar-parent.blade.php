...
<div class="flex my-4 items-start">
    <div id="sidebar-parent" class="w-28 md:block hidden md:w-80 h-full -translate-x-full transition-all bg-yellow-200 rounded-xl md:py-8 py-2 px-2 md:px-4 gap-4">
        <a href="dashboard-parents" class=" flex items-center gap-3 px-4 py-2 hover:bg-yellow-100 rounded-md {{request()->is('parents/dashboard-parents') ? 'bg-yellow-100' : ''}}">
            <img src="{{ asset('images/image-3.svg') }}" alt="home"/>
            <span class="text-black text-base hidden md:block">Dashboard</span>
        </a>
        <div class="relative">
            ...
            <div id="tutor-menu-parent" class="hidden md:flex flex-col ml-7 space-y-2 rounded p-4">
                <a href="find-tutor-parent" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('parents/find-tutor-parent') ? 'bg-yellow-100' : ''}}">Cari Tutor</a>
                <a href="tutor-applicants-parents" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('parents/tutor-applicants-parents') ? 'bg-yellow-100' : ''}}">List Pelamar Tutor</a>
                <a href="tutor-management" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('parents/tutor-management') ? 'bg-yellow-100' : ''}}">Tutor Status</a>
            </div>
        </div>
        <a href="parentsprofile" class=" flex items-center gap-3 px-4 py-2 hover:bg-yellow-100 rounded-md {{request()->is('parents/parentsprofile') ? 'bg-yellow-100' : ''}}">
            <img src="{{ asset('images/image-8.svg') }}" alt="home"/>
            <span class="text-black text-base hidden md:block">Profile</span>
        </a>
