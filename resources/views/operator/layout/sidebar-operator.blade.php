...
<div class="flex my-4 items-start">
    <div id="sidebar-parent" class="w-28 md:block hidden md:w-80 h-full -translate-x-full transition-all bg-yellow-200 rounded-xl md:py-8 py-2 px-2 md:px-4 gap-4">
        <a href="dashboard" class=" flex items-center gap-3 px-4 py-2 hover:bg-yellow-100 rounded-md {{request()->is('operator/dashboard') ? 'bg-yellow-100' : ''}}">
        ...
        <div class="relative">
            ...
            <div id="operator-tutormanagement-menu" class="hidden md:flex flex-col ml-7 space-y-2 rounded p-4">
                <a href="operator-tutor-status" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('operator/operator-tutor-status-detail') ? 'bg-yellow-100' : ''}}">Tutor Status</a>
                <a href="operator-tutor-criteria-inbox" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('operator/operator-tutor-criteria-inbox-detail') ? 'bg-yellow-100' : ''}}">Tutor criteria inbox</a>
                {{-- <a href="operator-tutor-review" class="text-black hover:bg-yellow-100 p-2 rounded-md {{request()->is('operator/operator-tutor-review') ? 'bg-yellow-100' : ''}}">Tutor review</a> --}}
            </div>
        </div>
        <a href="operator-profil" class=" flex items-center gap-3 px-4 py-2 hover:bg-yellow-100 rounded-md {{request()->is('parents/parentsprofile') ? 'bg-yellow-100' : ''}}">
